...
public class checkuser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Ingreso de usuarios</title>");            
            out.println("</head>");
            out.println("<body>");
            ...
            while (rs.next()){
                out.println("<h1>Usuario valido: " + rs.getString("usuario") + "</h1>");
                out.println("<h1>Nombre y apellido: " + rs.getString("nombreyapellido") + "</h1>");
            }
                
            if ( rs.first() ==  false)
            {
                out.println("<h1>No hay usuarios que coincidan con la base</h1>");
               // out.println("<h1>Servlet checkuser at " + request.getContextPath() + "</h1>");
                out.println("<h1>El Email ingresado es: " + request.getParameter("inputEmail") + "</h1>");
            }
